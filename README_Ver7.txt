# Usng Generate_Test_Case_Ver7.py

Steps to use:

1) run Truth_Table.py to create permutations of 'test steps'. Modify formula of interlocking logic here as required. 

2) Example input file with 'test steps' generated in 1) is provided: Expanded_test1.xlsx

3) Use associated SCL file '5.1-20230131_NUCBX1.scd' 

This file comes with a GUI select your own signal list and scl file. Also, permutations of test steps can be generatedautomatically by using Truth_Table.py.   